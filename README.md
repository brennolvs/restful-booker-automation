...
│   └── BUG_REPORT_TEMPLATE.md    # Template de 16 campos para documentar bugs
├── src/
...
npm run test:perf
```
- `npm run test:perf`: smoke test de performance via k6 (prova de conceito). **Não roda no CI** — da uma olhada `docs/TEST_PLAN.md` seção 8 para a limitação conhecida (a API pública de demo bloqueia tráfego do k6 com `418`).

## CI/CD

Todo push ou pull request para `main` dispara o workflow `.github/workflows/playwright.yml`, que
instala as dependências, roda a suíte completa e publica o relatório HTML como artefato do run.
